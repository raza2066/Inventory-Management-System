...
function displayPurchaseDetails() {
	...
	document.forms["form1"].method = "post"; document.forms["form1"].submit();
}
function callFun() {
...
function disableChequeNo(tableID) {
	...
	var rowCount = table.rows.length; var x = 0;
	for (var i = 1; i < rowCount; i++) {
		var row = table.rows[i]; x = (i - 1) * 10 + 2;
...
function addPaymentRow(tableID) {
	...
	if (table != null) {
		...
		for (var i = 1; i < rowCount; i++) {
			var row = table.rows[i]; if (row.children[1].firstElementChild.defaultValue != '') {
...
function save_row(tableID) {
	...
	for (var i = 1; i < rowCount; i++) {
		...
		if (null != chkbox && true == chkbox.checked) {
			...
			x = (i - 1) * 10 + 6; totalAmt = Number(totalAmt) + (Number(row.children[5].firstChild.form[x].value) - Number(oldPaidAmt));
		}
	} if (count <= 0) {
...
function callSave(tableID) {
	var table = document.getElementById(tableID);
	var rowCount = table.rows.length; var totalAmt = 0.0;
	var balance = document.getElementById('bal').value;
	for (var i = 1; i < rowCount; i++) {
		var row = table.rows[i];
		var chkbox = row.cells[0].childNodes[0]; if (null != chkbox && true == chkbox.checked) {
			row.children[8].firstElementChild.defaultValue = 'Y'; if (row.children[1].firstElementChild.defaultValue != '') {
				row.children[2].children[1].setAttribute("disabled", "true");
			} else {
				row.children[2].children[1].setAttribute("disabled", "true");
			}
			...
			if (row.children[1].firstElementChild.defaultValue != '') {
				if (row.children[2].children[1].getAttribute('disabled') != "true") { row.children[2].children[0].setAttribute("disabled", "true"); }
			} else {
				row.children[2].children[1].setAttribute("disabled", "true");
			}
		}
	...
	document.forms["form2"].method = "post"; document.forms["form2"].submit();
}
function deletePaymentRow(tableID) {
	var table = document.getElementById(tableID);
	var rowCount = table.rows.length; var count = 0; for (var i = 1; i < rowCount; i++) {
		var row = table.rows[i]; var chkbox = row.cells[0].childNodes[0];
		if (null != chkbox && true == chkbox.checked) {
			count = count + 1;
			if (row.children[1].firstElementChild.defaultValue != '') {
...
function editPaymentRow(tableID) {
	...
	var table = document.getElementById(tableID);
	var rowCount = table.rows.length;
	var count = 0; for (var i = 1; i < rowCount; i++) {
		var row = table.rows[i];
		...
		document.getElementById("addbtn").disabled = true; document.getElementById("deletebtn").disabled = true;
		document.getElementById("edit_button").disabled = true;
		for (var i = 1; i < rowCount; i++) {
			var row = table.rows[i];
			var chkbox = row.cells[0].childNodes[0]; row.children[0].firstElementChild.setAttribute("disabled", "true");
		}
	} else { alert("Please select atleast one row to edit the data."); return; }
}
